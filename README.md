# Julius Busecke personal homepage
...
The old template used this strictly for languages, but I find it pretty and practical to extend this to tools (e.g. python libraries like xarray and dask). The "Tech Stack" bubble on the index page is populated dynamically from the data defined in `/src/utils/languages.ts`. To add a new tool simply add an entry (and a new icon in `/src/icons/`)
